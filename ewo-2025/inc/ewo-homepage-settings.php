...
function ewo_2025_hps_handle_save() {
	...
	// 1. Update show/hide in the shared Feature Visibility option.
	$fv_keys  = array( 'latest_analysis', 'featured_videos', 'strategic_domains', 'predictions' );
	$fv_saved = get_option( EWO_2025_FEATURE_VIS_OPTION, array() );
	if ( ! is_array( $fv_saved ) ) {
		$fv_saved = array();
	}
	$show_raw = isset( $raw['show'] ) && is_array( $raw['show'] ) ? $raw['show'] : array();
	foreach ( $fv_keys as $key ) {
		$fv_saved[ sanitize_key( $key ) ] = isset( $show_raw[ $key ] ) ? 1 : 0;
	}
	update_option( EWO_2025_FEATURE_VIS_OPTION, $fv_saved );

	// 2. Save source/count settings to their own option.
...
function ewo_2025_hps_render_page() {
	...
	$fv_saved  = get_option( EWO_2025_FEATURE_VIS_OPTION, array() );
	$fv_defs   = function_exists( 'ewo_2025_fv_defaults' ) ? ewo_2025_fv_defaults() : array();
	$fv        = is_array( $fv_saved ) ? array_merge( $fv_defs, $fv_saved ) : $fv_defs;
	$saved_msg = isset( $_GET['saved'] ) && '1' === sanitize_text_field( wp_unslash( $_GET['saved'] ) ); // phpcs:ignore WordPress.Security.NonceVerification

	// Show/hide state for homepage sections managed here.
	$show = array(
		'latest_analysis'    => ! empty( $fv['latest_analysis'] ),
		'featured_videos'    => ! empty( $fv['featured_videos'] ),
		'strategic_domains'  => ! empty( $fv['strategic_domains'] ),
		'predictions'        => ! empty( $fv['predictions'] ),
	);
	?>
	<style>
	...
	.hs-toggle-row{align-items:center;border-bottom:1px solid var(--hs-border2);display:flex;gap:14px;padding:12px 18px}
	.hs-toggle-row:last-child{border-bottom:0}
	.hs-toggle-row__info{flex:1}
	.hs-toggle-row__label{color:var(--hs-white);font-size:.85rem;font-weight:600}
	.hs-toggle-row__desc{color:var(--hs-muted);font-size:.75rem;margin-top:2px}
	.hs-tw{align-items:center;display:flex;flex-shrink:0;gap:8px}
	.hs-tgl{position:relative;display:inline-block;width:44px;height:24px}
	.hs-tgl input{opacity:0;width:0;height:0;position:absolute}
	.hs-sld{position:absolute;cursor:pointer;inset:0;background:rgba(50,100,160,.3);border-radius:24px;transition:background .2s}
	.hs-sld::before{position:absolute;content:"";height:18px;width:18px;left:3px;bottom:3px;background:var(--hs-muted);border-radius:50%;transition:transform .2s,background .2s}
	.hs-tgl input:checked+.hs-sld{background:rgba(74,222,128,.22)}
	.hs-tgl input:checked+.hs-sld::before{transform:translateX(20px);background:var(--hs-green)}
	.hs-ts{color:var(--hs-muted);font-size:.72rem;font-weight:600;min-width:54px;text-align:right;text-transform:uppercase}
	.hs-ts--on{color:var(--hs-green)}
	.hs-note{color:var(--hs-muted);font-size:.76rem;font-style:italic;padding:10px 18px 14px}
	...
	<script>
	function ewsToggle(el){
		var tw=el.closest('.hs-tw');
		var ts=tw.querySelector('.hs-ts');
		if(el.checked){ts.textContent='Enabled';ts.classList.add('hs-ts--on');}
		else{ts.textContent='Disabled';ts.classList.remove('hs-ts--on');}
	}
	</script>
		...
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
			...
			<div class="hs-row">
				<div class="hs-card hs-card--full">
					<div class="hs-card__head"><h2><?php esc_html_e( 'Section Visibility', 'ewo-2025' ); ?></h2></div>
					<?php
					$visibility_rows = array(
						'latest_analysis'   => array( __( 'Latest Analysis', 'ewo-2025' ),   __( 'Research & Analysis section — WordPress articles and/or Substack imports', 'ewo-2025' ) ),
						'featured_videos'   => array( __( 'Strategic Playlists', 'ewo-2025' ), __( 'YouTube playlists carousel (§8)', 'ewo-2025' ) ),
						'strategic_domains' => array( __( 'Strategic Domains', 'ewo-2025' ),  __( 'Smart Feed domain cards (§5)', 'ewo-2025' ) ),
						'predictions'       => array( __( 'Strategic Predictions', 'ewo-2025' ), __( 'Forecast cards from the Predictions plugin (§6)', 'ewo-2025' ) ),
					);
					foreach ( $visibility_rows as $key => list( $label, $desc ) ) :
						$on = ! empty( $show[ $key ] );
					?>
					<div class="hs-toggle-row">
						<div class="hs-toggle-row__info">
							<div class="hs-toggle-row__label"><?php echo esc_html( $label ); ?></div>
							<div class="hs-toggle-row__desc"><?php echo esc_html( $desc ); ?></div>
						</div>
						<div class="hs-tw">
							<label class="hs-tgl">
								<input type="checkbox" name="ewo_hps[show][<?php echo esc_attr( $key ); ?>]" value="1" <?php checked( $on ); ?> onchange="ewsToggle(this)">
								<span class="hs-sld"></span>
							</label>
							<span class="hs-ts<?php echo $on ? ' hs-ts--on' : ''; ?>"><?php echo $on ? esc_html__( 'Enabled', 'ewo-2025' ) : esc_html__( 'Disabled', 'ewo-2025' ); ?></span>
						</div>
					</div>
					<?php endforeach; ?>
					<p class="hs-note"><?php esc_html_e( 'All other sections (YouTube Slider, Community Wall, Book, Platform Network, header chips, footer, sidebar) are toggled in EWO Settings → Feature Visibility.', 'ewo-2025' ); ?></p>
				</div>
			</div>
